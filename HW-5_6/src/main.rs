...
impl ToyMMU {
    ...
    pub fn translate(&self, va: u8, is_write: bool) -> Result<u8, MemoryError> {
        ...
        let virtual_page_number = va >> 3; // получаем старшие биты
        let offset = va & 7; // получаем младшие биты с помощью маски 0b111


        let page = self.page_table[virtual_page_number as usize];
        
        let user_flag = (page & 4) != 0; 
        let write_flag = (page & 2) != 0;
        let present_flag = (page & 1) != 0;

        if !present_flag {
            return Err(MemoryError::PageNotPresent);
        }

        if !write_flag && is_write {
            return Err(MemoryError::WriteProtected);
        }

        if !user_flag && self.current_privilege == PrivilegeLevel::Ring3 {
            return Err(MemoryError::PrivilegeViolation)
        }

        let physical_page_number = page & 56;
